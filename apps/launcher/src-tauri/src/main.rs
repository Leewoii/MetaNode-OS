...
fn check_access(device: &DeviceState) -> Result<String, String> {
  let gate_url = env_or_default("METANODE_GATE_URL", "https://metanode-access-gate.example.workers.dev");
...
fn ensure_core_bundle(app: &AppHandle, grant: &str) -> Result<(), String> {
  ...
  let gate_url = env_or_default("METANODE_GATE_URL", "https://metanode-access-gate.example.workers.dev");
  let distribution_url = env_or_default("METANODE_DISTRIBUTION_URL", "https://metanode-distribution.example.com");
  let client = reqwest::blocking::Client::new();
