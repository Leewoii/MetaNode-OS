...
const env = {
  port: Number(process.env.DISTRIBUTION_PORT ?? 4320),
...
app.get("/v1/core/releases/latest", async () => {
...
app.get("/v1/core/assets/:assetId", async (request, reply) => {
  const token = verifySignedPayload(bearer(headerValue(request.headers.authorization)), env.gateSecret);
